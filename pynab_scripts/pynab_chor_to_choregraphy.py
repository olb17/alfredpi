...
class ChoreographyInterpreter:
    ...
    LEDS = {
        0: "Led4",
        1: "Led3",  # when looking at the rabbit
        2: "Led2",
        3: "Led1",
        4: "Led0",
    }
...
#         self.leds.set1(led, r, g, b)
        print(f"led_color ~{r:02x}{g:02x}{b:02x}, {led}")
        return index + 6
...
#        await self.ears.go(motor, position, direction)
        motor = "Left" if motor == 0 else "Right"
        print(f"set_ear {motor}, {position}")
        return index + 3
    ...
    async def set_leds_color(self, index, chor):
        ...
        g = chor[index + 1]
        b = chor[index + 2]
        print(f"led_color ~{r:02x}{g:02x}{b:02x}, Led0")
        print(f"led_color ~{r:02x}{g:02x}{b:02x}, Led1")
        print(f"led_color ~{r:02x}{g:02x}{b:02x}, Led2")
        print(f"led_color ~{r:02x}{g:02x}{b:02x}, Led3")
        print(f"led_color ~{r:02x}{g:02x}{b:02x}, Led4")
    ...
    async def avance(self, index, chor):
        ...
        if direction == 0:
            direction = -1
        motor = "Left" if motor == 0 else "Right"
        print(f"move_ear {motor}, {delta * direction}")
    ...
    async def do_play_binary(self, start_index, chor, opcodes, timescale):
        ...
        while index < len(chor):
            ...
            if sleep_delta > 0:
                print(f"sleep {round(sleep_delta * 1000)}")
...
if __name__ == "__main__":
    ...
    # Example usage, replace with actual path to a .chor file.
    # This is just a placeholder to demonstrate how to run the interpreter.
    inter = ChoreographyInterpreter()
    arg = sys.argv[1] #"nabtaichid/choreographies/nabtaichid/taichi.chor" 
    asyncio.run(inter.play(arg))
